...
async function deployTestnetFixedGas() {
    console.log("🚀 DÉPLOIEMENT SUR LE TESTNET HYPEREVM (GAS CORRIGÉ)...");
    ...
    const rpcUrl = "https://998.rpc.thirdweb.com";
    ...
    try {
        ...
        while (retryCount < maxRetries) {
            try {
                // Prix de gas plus élevés pour le testnet
                marketplace = await marketplaceFactory.deploy(
                    ...
                    nativeTokenWrapper,
                    {
                        gasLimit: 10000000, // Limite de gas élevée
                        gasPrice: ethers.parseUnits("0.000000001", "gwei") // Prix de gas plus élevé
                    }
                );
                break;
            } catch (error) {
        ...
        console.log("   Gas Limit: 10,000,000 (Marketplace), 5,000,000 (Extensions)");
        console.log("   Gas Price: 0.000000001 gwei (approprié pour le testnet)");
...
if (import.meta.url === `file://${process.argv[1]}`) {
    deployTestnetFixedGas().catch((error) => {
