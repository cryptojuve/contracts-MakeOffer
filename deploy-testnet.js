...
async function deployTestnet() {
    console.log("🚀 DÉPLOIEMENT SUR LE TESTNET HYPEREVM...");
    ...
    // Utiliser le testnet HyperEVM (Chain ID 998)
    const rpcUrl = process.env.HYPEREVM_TESTNET_RPC_URL || "https://998.rpc.thirdweb.com";
    ...
    try {
        ...
        if (balance < ethers.parseEther("0.00001")) {
            ...
            console.log("💡 Sur le testnet, vous devriez avoir des tokens gratuits");
            console.log("💡 Vérifiez votre configuration RPC testnet");
            process.exit(1);
        }
        ...
        while (retryCount < maxRetries) {
            try {
                marketplace = await marketplaceFactory.deploy(
                    ...
                    nativeTokenWrapper,
                    {
                        gasLimit: 5000000 // Limite de gas élevée pour le testnet
                    }
                );
                break;
            } catch (error) {
        ...
        console.log("💡 Ces contrats sont maintenant prêts à être testés sur le testnet !");

        console.log("\n🧪 PROCHAINES ÉTAPES:");
        console.log("   1. Testez les fonctions d'extension");
        console.log("   2. Vérifiez que addExtension fonctionne");
        console.log("   3. Une fois validé, déployez sur le mainnet");
