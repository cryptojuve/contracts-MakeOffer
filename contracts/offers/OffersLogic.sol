...
import "lib/creator-token-standards/lib/openzeppelin-contracts/contracts/utils/introspection/IERC165.sol";
import "lib/contracts/contracts/eip/interface/IERC721.sol";
import "lib/creator-token-standards/lib/openzeppelin-contracts/contracts/token/ERC1155/IERC1155.sol";
import "lib/creator-token-standards/lib/openzeppelin-contracts/contracts/token/ERC20/IERC20.sol";
...
contract OffersLogic is IOffers, ReentrancyGuard, ERC2771ContextConsumer {
    ...
    function _validateERC20BalAndAllowance(address _tokenOwner, address _currency, uint256 _amount)
        internal
        view
        returns (bool isValid)
    {
        isValid = IERC20(_currency).balanceOf(_tokenOwner) >= _amount
    ...
    function _payout(
        ...
        // Payout platform fee
        {
            ...
            CurrencyTransferLib.transferCurrencyWithWrapper(
                _currencyToUse, _payer, DEFAULT_FEE_RECIPIENT, platformFeesTw, address(0)
            );
            CurrencyTransferLib.transferCurrencyWithWrapper(
                _currencyToUse, _payer, platformFeeRecipient, platformFeeCut, address(0)
            );
        ...
        // Payout royalties
        {
            ...
            (address payable[] memory recipients, uint256[] memory amounts) =
                RoyaltyPaymentsLogic(address(this)).getRoyalty(_offer.assetContract, _offer.tokenId, _totalPayoutAmount);
            ...
            if (royaltyRecipientCount != 0) {
                ...
                for (uint256 i = 0; i < royaltyRecipientCount;) {
                    ...
                    // Transfer royalty
                    CurrencyTransferLib.transferCurrencyWithWrapper(
                        _currencyToUse, _payer, royaltyRecipient, royaltyCut, address(0)
                    );
